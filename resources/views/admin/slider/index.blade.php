      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box box-info">
            ...
            <a class="btn btn-success btn-sm" href="/admin/slider/add-slider" title="Add Slider"> <i class="ace-icon fa glyphicon-plus white"></i> Add New </a>
              ...
              <table id="example1" class="table table-bordered table-striped">
				...
				$i = '1';
					foreach($recordset as $row)
					{
						...
						$editLink = '/admin/slider/edit-slider/'.$row->id;
				?>

                <tr id="tr<?php echo $row->id;?>">
                  <td><?php echo $i; ?></td>
                  <td><img src="{{URL::to('uploads/slider/'.$row->image)}}" height="150" width="750"></td>
                  ...
                  <td><a class="btn btn-xs btn-info" href="<?php echo $editLink;?>" title="Edit <?php echo ucfirst($row->name);?>"> <i class="ace-icon fa fa-pencil bigger-120"></i> </a>

                  <a class="btn btn-xs btn-danger" href="javascript:void(0);" title="Delete <?php echo ucfirst($row->name);?>" onclick="delete_user('<?php echo $row->id; ?>');"> <i class="ace-icon fa fa-trash-o bigger-120"></i> </a>
                   </td>
...
function change_status(id){
	...
	{
		$.ajax({
			url : baseurl+'/admin/slider/change-status',
			type : 'POST',
			data : {'slider_id':id},
			//dataType : 'json',
...
function delete_user(id,editID){
	if(confirm("Are you sure to delete of this Slider ?"))
	{
		$.ajax({
			url : baseurl+'/admin/slider/delete-slider',
			type : 'POST',
			data : 'id=' + id,
			//dataType : 'json',
			beforeSend : function(jqXHR, settings ){
