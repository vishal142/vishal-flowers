...
var isBookmarkUrl = true;
var preSelectedDeliveryDate = null;
var preSelectedPriceSlider = null;
var lastScrollTopValue = 0;
/**
 * @Auth Chitrarth This is written for search functionality in delivery city
 */
function cityData(cityName, cityLink) {
	this.display = cityName;
	this.link = cityLink;
}
// fnpApp is defined in fnp.js file
fnpApp.controller('SearchController', [ '$scope', function($scope) {
	var cityName = $("#citydata").html().replace(/\n/g, "").split(",");
	var cityObjFirstCol = [];
	//var cityObjSecondCol = [];
	var count=0;
	for(var i=0,j=0; i<cityName.length;i=i+2){
		if(cityName[i] != null && cityName[i+1] != null){
			var cityDisplay = cityName[i].trim();
			var cityLink	= cityName[i+1];
			cityObjFirstCol[j] = new cityData(cityDisplay,cityLink);
			j++;
		}
	}
	$scope.firstColCities = cityObjFirstCol;
 }]);
...
fnpApp.constant("SUBMENU_LENGTH", 7).config([ '$compileProvider', function($compileProvider) {
	// fster
	$compileProvider.debugInfoEnabled(false);
} ]).controller("ProductListingController", ProductListingController).factory("MenuService", MenuService).factory("ProductListingService",
		ProductListingService).directive('jqdatepicker', function() {
	return {
		restrict : 'A',

		require : 'ngModel',
		link : function(scope, element, attrs, ngModelCtrl) {
			deliveryDtls = scope.deliveryDateDtls;
			scope.deliveryDateFieldName = deliveryDtls.deliveryDateFieldName;
			element.datepicker({
				dayNamesMin : [ 'S', 'M', 'T', 'W', 'T', 'F', 'S' ],
				minDate : (new Date().getHours()) < parseInt(17) ? 0 : 1,
				dateFormat : dateFormat,
				autoclose : true,
				onSelect : function(date) {
					$("input#deliverydatefilter").val(date);
					scope.date = date;
			...
			$(".ui-datepicker").addClass("smalldatepicker");
			if (scope.deliveryDate || deliveryDtls.deliveryDate) {
				element.datepicker("setDate", deliveryDtls.deliveryDate);
			}
		}
	};
}).directive('triggerjqueryevents', [ '$timeout','$rootScope', function($timeout,$rootScope) {
	return {
		link : function($scope, element, attrs) {
			$scope.$on('dataloaded', function() {
				$rootScope.isBusy = false;
				$timeout(function() { // You might need this timeout to be sure its run after DOM render.
					// ajaxindicatorstop(); // this will close over lay in category and search pages.
					initCurrencies();
				}, 0, false);
			})
		}
...
/*
 * directive('jqpriceslider', function() { return { restrict : 'A', link : function(scope, element, attrs) { scope.maxPrice = app.priceSliderData.maxPrice;
 * scope.startPrice = app.priceSliderData.selectedStartRange; scope.endPrice = app.priceSliderData.selectedEndRange; priceFieldName
 *
 * priceData = scope.priceSliderData; console.log(scope.priceSliderData); element.slider({ range : true, min : priceData.minStartRange, max :
 * priceData.maxEndRange, values : [ priceData.selectedStartRange, priceData.selectedEndRange], slide : function(e, ui) { //console.log("slide event triggered:
 * start" + ui.values[0] + ", end :" + ui.values[1]); }, change : function(e, ui) { scope.min = ui.values[0]; scope.max = ui.values[1];
 * setPriceSliderValue(scope.min, scope.max); $("input[name='PRICE_AED']:checked").prop("checked", false);//TODO - making unselect another price ranges if
 * already selected. scope.fetchProducts(false); if ((ui.values[0] + 700) >= ui.values[1]) { return false; } scope.$apply(); }, create : function(e, ui) {
 * scope.min = priceData.selectedStartRange; scope.max = priceData.selectedEndRange; scope.fieldName = priceData.priceFieldName; } }); } }; })
 */

/*
 * function setPriceSliderValue(min, max){ $("input#priceSlider").val("[" + min + " TO " + max + "]"); }
 */

// model
function ProductListingService($resource) {
	return {
		fetchProductListingAndFacet : fetchProductListingAndFacet,
		getProducts : getProducts,
		loadProductListingDefaults : loadProductListingDefaults
	};
	function fetchProductListingAndFacet(fetchURL) {
		console.log(fetchURL);
		var request = $resource(fetchURL);
		return request.get().$promise;
	}
	function loadProductListingDefaults() {
		var listingConfig = angular.fromJson($("section[data-product-listing-defaults]").data("product-listing-defaults"));
		if (typeof listingConfig == 'undefined') {
			listingConfig = {};
		}
		$.extend(ProductListingDefaults, listingConfig);
...
function MenuService($resource) {
	return {
		...
		checkHashFilters : checkHashFilters,
		getUpdatedFilterFetchURL : getUpdatedFilterFetchURL,
		prepareFilterQueryString : prepareFilterQueryString,
	...
	function getBrowseRootCategories() {
		return [];
	}
	...
	function genericFilters() {
		// fetch the filters from dom - with id 'filtersOptions'..
		// filters will updated as and when a filter selection is made..
		// $("#filtersOptions")
		return angular.fromJson($("#filtersOptions").html());
	}
	...
	function checkHashFilters(filtersJson, deliveryDtls, priceSliderDtls) {
		console.log("filtersJson:" + filtersJson);
		if ($.deparam.fragment().fq) {
			if (deliveryDtls) {
				$(fqParams).each(function(i2, selectedValue) {
					var splitValue = selectedValue.split(":");
					var name = splitValue[0];
					var val = splitValue[1];
					if (deliveryDtls.deliveryDateFieldName == name) {
						deliveryDtls.deliveryDate = val;
						preSelectedDeliveryDate = selectedValue;
					}
				});
				return deliveryDtls;
			} else if (priceSliderDtls) {
				$(fqParams).each(function(i2, selectedValue) {
					var splitValue = selectedValue.split(":");
					var name = splitValue[0];
					var val = splitValue[1];
					if (priceSliderDtls.priceFieldName == name) {
						preSelectedPriceSlider = selectedValue;
						var patt = /[0-9]+/g;
						var arry = [];
						while (true) {
							...
							if (result == null)
								break;
							arry.push(parseInt(result.toString()));
						}
						priceSliderDtls.selectedEndRange = arry[1];
						priceSliderDtls.selectedStartRange = arry[0];
					}
				});
				return priceSliderDtls;
			} else {
				$(filtersJson).each(function(i1, filterValue) {
					fqParams = [];
					if (!$.isArray($.deparam.fragment().fq)) {
						fqParams.push($.deparam.fragment().fq);
					} else {
						fqParams = $.deparam.fragment().fq;
					}
					$(fqParams).each(function(i2, selectedValue) {
						var splitValue = selectedValue.split(":");
						var name = splitValue[0];
						var val = splitValue[1];
						if (filterValue.filterFacetField == name) {
							$(filterValue.filterOptions).each(function(i3, filterField) {
								if (filterField.value == val) {
	...
	function getUpdatedFilterFetchURL(allFilters, app) {
		 /*if(($("#FNP_CURRENT_CATALOG_ID").val().toUpperCase() != "uae".toUpperCase())){
			 var filterURL = "/control/internationalProductlisting" + ProductListingDefaults.format;
		 }else{
			 var filterURL = "/control/productlisting" + ProductListingDefaults.format;
		 }*/
		var filterURL = "/control/productlisting" + ProductListingDefaults.format;
		var finalQueryString = prepareFilterQueryString(allFilters, true, false, app);
		// var pagePath = window.location.href.substring(0,window.location.href.indexOf("#"));
		// window.location.href = pagePath+"#"+queryString.join("&");
		// initiate a filter ajax call.. add the format as well to json url
		filterURL = filterURL + finalQueryString
		var excludeParamString = getExcludeParams();
		if(excludeParamString){
			filterURL = filterURL + "&" + excludeParamString;
		}
		
		filterURL = filterURL + "&format=" + ProductListingDefaults.format;
		$.bbq.pushState(finalQueryString, 2);
		return filterURL;
	}

	function getExcludeParams(){
		var excludeParams = [ "viewIndex", "viewSize", "sortFields" ,"fq" ]
		var paramString = "";
		var parameters = $.deparam.querystring(true);
	...
	// Method to get the filter url for bookmark url. so not pushing the values to bbq plugin.
	function getFiltersFromUrl(allFilters, app) {
		 /*if(($("#FNP_CURRENT_CATALOG_ID").val().toUpperCase() != "uae".toUpperCase())){
			 var filterURL = "/control/internationalProductlisting" + ProductListingDefaults.format;
		 }else{
			 var filterURL = "/control/productlisting" + ProductListingDefaults.format;
		...
		var filterURL = "/control/productlisting" + ProductListingDefaults.format;
		var finalQueryString = prepareFilterQueryString(allFilters, true, true, app);
		filterURL = filterURL + finalQueryString;
		var excludeParamString = getExcludeParams();
		if(excludeParamString){
			filterURL = filterURL + "&" + excludeParamString;
	...
	function prepareFilterQueryString(allFilters, includeDefaultFilter, isBookmarkUrl, app) {
		var filterOptionsChecked = $(".filterOptionsSelection .checkbox");
		var queryString = null;
		// if its a bookmarkurl get all the list of products which are there earlier.
		if (isBookmarkUrl) {
			var currentViewIndex = parseInt($.urlParam("viewIndex"));
			var currentViewSize = parseInt($.urlParam("viewSize"));
			currentViewSize = (currentViewIndex + 1) * currentViewSize;
			queryString = [ "viewIndex=" + currentViewIndex, "viewSize=" + currentViewSize ];
			// update view index so next click on load more will get sync
			ProductListingDefaults.viewIndex = currentViewIndex;
		} else {
			queryString = [ "viewIndex=" + ProductListingDefaults.viewIndex, "viewSize=" + ProductListingDefaults.viewSize ];
		}
		if ($("section[data-page-filters]").length > 0 && includeDefaultFilter) {
			var defaultFilters = $("section[data-page-filters]").data("page-filters");
			queryString.push($.param(defaultFilters, true));
		}
		/*
		 * if (filterOptionsChecked.find(":checked").size()) { $("a.mega").find("label.active").show(); filterOptionsChecked.find(":checked").each(function() {
		 * $(this).parent().siblings("label.checklabel").addClass("checkedlabel"); queryString.push("fq="+$(this).attr("name")+":"+$(this).val()); }); }
		 */
		angular.forEach(allFilters, function(filter, genericFilterIndex) {
			// console.log(genericFilter);
			angular.forEach(filter.filterOptions, function(filterOption, filterOptionIndex) {
				// console.log(genericFilter);
				// console.log(filterOption);
				if (filterOption.selected) {
					this.push("fq=" + filter.filterFacetField + ":" + filterOption.value);
				}
			}, queryString);
		}, queryString);
		
		//price filter fq include
		if(app.selectedPrice){
			queryString = $.grep(queryString, function(value) {
			    return value.indexOf("fq=PRICE_SGD:") < 0;
			});
			queryString.push("fq=PRICE_SGD:"+app.selectedPrice);
		} else {
			var priceFromUrl = $.grep(queryString, function(value) {
			    return value.indexOf("fq=PRICE_SGD:") >= 0;
			});
			if(priceFromUrl && priceFromUrl.length > 0){
				var selectedValue = priceFromUrl[0];
				var splitValue = selectedValue.split(":");
				var val = splitValue[1];
				app.selectedPrice = val;
			}
		}
		...
		// price slider
		/*
		 * if($("input#priceSlider").val()){ queryString.push("fq="+$("input#priceSlider").attr('name')+":"+$("input#priceSlider").val()); } else
		 * if(preSelectedPriceSlider != null){ queryString.push("fq="+preSelectedPriceSlider); preSelectedPriceSlider = null; }
		 */

		// delivery date
		if ($("input#deliverydatefilter").val()) {
			/*var gfFinderCookie={};
			if($.cookie('gfFinderCookie')){		
				gfFinderCookie= JSON.parse($.cookie('gfFinderCookie'));
			}else{
			...
			if($("input#deliverydatefilter").val()!=null && $("input#deliverydatefilter").val()!=""){
				gfFinderCookie.date=deliveryDateConvToName($("input#deliverydatefilter").val());
				gfFinderCookie.city=gfFinderCookie.city;
		      $.cookie('gfFinderCookie', JSON.stringify(gfFinderCookie),{path : '/',domain:currentdomain});
			} */ 
			queryString.push("fq=" + $("input#deliverydatefilter").attr('name') + ":" + $("input#deliverydatefilter").val());
		} else if (preSelectedDeliveryDate != null) {
			queryString.push("fq=" + preSelectedDeliveryDate);
			preSelectedDeliveryDate = null;
		}

		// check if sort options are selected..
		if ($(".sortfieldsselectbox").val() != "") {
			queryString.push("sortFields=" + $(".sortfieldsselectbox").val());
		}
		// queryString.push();
		// var finalQueryString = queryString.join("&").replace(/:/g,"\\:");//ensure : (colon) is escaped as angular will treat : as port separator
		// finally also add/merge query string if any..
		var queryStringObject = jQuery.deparam.querystring(queryString.join("&"), true);
		// console.log(queryStringObject);
		// merge query string and fragment...
		var selectedParams = $.deparam.querystring(true);
		// console.log(selectedParams);
		// console.log($.param.querystring("?"+$.param.querystring(),queryStringObject,0));
		var finalQueryString = $.param.querystring("?", queryStringObject, 0);
		// var selectedParams = jQuery.deparam.querystring(queryString.join("&"),false);
...
// controller
function ProductListingController($location,$scope,$filter, $rootScope, MenuService, ProductListingService, SUBMENU_LENGTH) {
	var app = this;
	$rootScope.isBusy = false ;
	app.submenu_size = SUBMENU_LENGTH;
	app.prevent = function($event) {
		$event.stopPropagation()
		return false;
	}
	app.chunk = function(obj, array, chunk_size) {
		if (array) {
			var arrayCopy = angular.copy(array);
			obj.row = [];
			arrayCopy.forEach(function(e, i) {
				obj.row[i] = arrayCopy.splice(i, chunk_size);
			});
		}
	};
	app.getSelectedValue=function(container){
		return $("#"+container).find(':checkbox:checked').val();
	};
	$scope.loadMoreProduct = function () {
		if(!$rootScope.isBusy && app.hasMore){
            app.fetchProducts(true);
		}
	};
	app.initialize=function(){
		$scope.date = '';
		app.priceFieldName = "PRICE_SGD"; // it will over ride when price range tab populated.
		app.deliveryDateFieldName = "deliveryDate";

		app.defaultProducts = {
			display : true
		};
		var productListingFilters = MenuService.genericFilters();
		var rightFilters = ($.deparam.fragment().fq) ? MenuService.checkHashFilters(productListingFilters.facetFieldFilterOptions)
				: productListingFilters.facetFieldFilterOptions;// right filters
		var leftFilters = ($.deparam.fragment().fq) ? MenuService.checkHashFilters(productListingFilters.facetFieldTagOptions)
				: productListingFilters.facetFieldTagOptions;// left filters;

		app.genericFilters = rightFilters;
		app.facetFieldTagOptions = leftFilters;
		app.priceFilters = ($.deparam.fragment().fq) ? MenuService.checkHashFilters(productListingFilters.facetRangeOptions)
				: productListingFilters.facetRangeOptions;
		$scope.priceSliderData = ($.deparam.fragment().fq) ? MenuService.checkHashFilters(null, null, productListingFilters.priceSlider)
				: productListingFilters.priceSlider;
		$scope.deliveryDateDtls = ($.deparam.fragment().fq) ? MenuService.checkHashFilters(null, productListingFilters.deliveryDateDtls, null)
				: productListingFilters.deliveryDateDtls;

		// initialize with products
		app.products = angular.fromJson(ProductListingService.getProducts());


		// load product listing config
		ProductListingDefaults = ProductListingService.loadProductListingDefaults();
		// track counters
		app.loadedProductsCount = ProductListingDefaults.viewSize;
		app.totalProductsCount = ProductListingDefaults.listSize;
		app.hasMore = (app.loadedProductsCount < app.totalProductsCount);
		app.sortByField = ProductListingDefaults.sortByField;
		// get filters
		app.filters = MenuService.getFilterCategories();
	};

	// init the topmenu
	if(angular.isDefined(MenuService.genericFilters())){
		app.initialize();
	}

	app.getLength = function(obj) {
	    return Object.keys(obj).length;
	}

	app.reInitializeFoundation = function() {
		$(document).foundation();
	};

	app.productsAndFacetsFetched = function(data) {
		// console.log(data);
		// also ensure products are updated..
		if (!ProductListingDefaults.loadMore) {
			// hide any default loaded products and reset facets, counters and products
			app.defaultProducts = {
				display : false
			};// hide default products..
			$(".pidloadeddefault").remove();// Note: added here due to performance increases by angular with ng-if
			app.genericFilters = data.facetFieldFilterOptions;// update facets as filters changed..
			app.facetFieldTagOptions = data.facetFieldTagOptions;
			//app.priceFilters = data.facetRangeOptions;
			app.selectedPriceFilters = data.facetRangeOptions;

			app.products = angular.fromJson(data.productResults);
			app.loadedProductsCount = data.viewSize;
			$scope.priceSliderData = data.priceSlider;
			 // fetching the products view count - only fetch required
			var  productList=[];
			angular.forEach(app.products, function(product) {
				  this.push(product.productId);
				}, productList);
			updateProductInfo(productList);// update delivery date & product count
			
		} else {
			//app.products = angular.extend(angular.fromJson(app.products), angular.fromJson(data.productResults));// add the fetched list..
			//app.products = angular.fromJson(app.products);
			//TODO - above lines only shows the current pageindex products. check performance with below code
			app.products = app.products.concat(angular.fromJson(data.productResults));
			
			 // only fetch required data productId
			var productList=[];
			angular.forEach(data.productResults, function(product) {
				  this.push(product.productId);
				}, productList);
			updateProductInfo(productList); // update delivery date & product count
		
			
			app.loadedProductsCount = app.loadedProductsCount + data.viewSize;
		}

		$scope.$broadcast('dataloaded');
		app.totalProductsCount = data.listSize;
		// console.log("app.loadedProductsCount: "+app.loadedProductsCount);
		// console.log("app.totalProductsCount: "+app.totalProductsCount);
		app.hasMore = (app.loadedProductsCount < app.totalProductsCount);

	}
	app.fetchFailed = function(reason) {
		console.log(reason);
	}
	app.clearPriceFilters=function(){
		app.selectedPrice=false;
		angular.forEach(app.priceFilters[0].filterOptions, function(filter) {
			if(filter.selected){
				filter.selected = false;
			}
		});
		app.fetchProducts(false);
	}
	app.getSelectedPrice=function(){
		if(app.selectedPriceFilters && $filter('filter')(app.selectedPriceFilters[0].filterOptions,{selected:true}).length){
			var selected= $filter('filter')(app.selectedPriceFilters[0].filterOptions,{selected:true});
			if(selected.length>0){
				return "SGD"+ selected[0].priceStart + " + ";
			}
		}
	};
	
	app.selectedDeliveryDate=function(){
		return $("input#deliverydatefilter").val();
	};


	app.fetchProducts = function(loadMore, viewAll) {
		$rootScope.isBusy = true;
		// ajaxindicatorstart('');
		var allFilters = app.genericFilters.concat(app.facetFieldTagOptions).concat(app.priceFilters);
		ProductListingDefaults.loadMore = loadMore;// if true - it means we are continuing the product load..and not re-fetching as in filters
		// increase the counter if loadMore otherwise reset...
		viewAll = (typeof viewAll != 'undefined') ? viewAll : false;// !(typeof viewAll=='undefined');
		if (!viewAll) {
			var fetchURL = null;
			var currentViewIndex = $.urlParam("viewIndex") != undefined ? $.urlParam("viewIndex") : null;

			if ((loadMore && isBookmarkUrl) || (isBookmarkUrl && currentViewIndex == null)) {
				isBookmarkUrl = false;
			}
			// checking if loadmore is clicked after hitting the bookmarkurl. If it is then continue as normal.
			/*
			 * if(isBookmarkUrl && loadMore){ isBookmarkUrl=false; }
			 */
			if (isBookmarkUrl) {
				isBookmarkUrl = false;
				fetchURL = MenuService.getFiltersFromUrl(allFilters, app);
			} else {
				viewIndex = ProductListingDefaults.viewIndex = loadMore ? ProductListingDefaults.viewIndex + 1 : 0;
				fetchURL = MenuService.getUpdatedFilterFetchURL(allFilters, app);
			}
			MenuService.fetchProductListingAndFacet(fetchURL).then(app.productsAndFacetsFetched, app.fetchFailed);
		} else {
			ProductListingDefaults.viewIndex = 0;
			ProductListingDefaults.viewSize = ProductListingDefaults.listSize;
			window.location.href = window.location.pathname + MenuService.prepareFilterQueryString(allFilters, false, false, app) + "&" + $.param.querystring() + "&viewAll=Y&TRANSACTION_TIMEOUT=0";
		}
	}
	$scope.fetchProducts = app.fetchProducts;
	$scope.showFilter = function(filterName){
		 if(($("#FNP_CURRENT_CATALOG_ID").val().toUpperCase() == "sgp".toUpperCase())){
			return  true;
		 }else{
			if(filterName =="Price"){
				 return  true;
	...
	if (!$.isEmptyObject($.deparam.fragment())) {
		// there was fragment in url i.e we need to apply filters.. so run fetch products...
		app.fetchProducts(false);
	}
}
...
$(document).ready(function() {
	...
	initFilter();
	/*
  *@kp causing top menu to close on hover
	*/
	//$(".has-dropdown a").click(function(){
		//$(this).find("label.active").removeClass("cate");
		//$(this).find("label.active").addClass("hide");
		//$(this).parent().find(".f-dropdown").removeClass("open");
	//});
//	$("label.active").on('click', function(e) {
//		$(this).hide();
//		$("div.open ul li.active").removeClass("active");
//		if ($("div.open .checkbox input[type='checkbox']").size()) {
//			$("label.checklabel").removeClass("checkedlabel");
...
//		$("a.mega").find("label.active").hide();
//		$("label.checklabel").removeClass("checkedlabel");
//		if ($(".checkbox").find(":checked").size()) {
//			$("a.mega").find("label.active").show();
//			$(".checkbox").find(":checked").each(function() {
//				$(this).parent().siblings("label.checklabel").addClass("checkedlabel");
//			})
//		}
//	});
//
...
function initFilter() {
	/*
	 * if ($('.top-bar-section .scroll-pane')) $('.top-bar-section .scroll-pane').jScrollPane({ verticalGutter : 10, showArrows : false, verticalDragMinHeight :
	 * 20, verticalDragMaxHeight : 20, verticalDragMinWidth : 5, verticalDragMaxWidth : 5, autoReinitialise : true });
	 */
	...
	function showDropDown() {
		...
		if ($(this).hasClass("mega")) {
			$("li.has-dropdown").children("a.mega").triggerHandler("click");
			$("li.has-dropdown").children("a.mega").addClass('hover');
		} else {

			$(this).parents("li.has-dropdown").children("a").triggerHandler("click");
			$(this).parents("li.has-dropdown").children("a").addClass('hover');
		}
	...
	function hideDropDown() {
		if ($(this).hasClass("mega")) {
			$("li.has-dropdown").children("a.mega").removeClass('hover');

		} else {
			$(this).parents("li.has-dropdown").children("a").removeClass('hover');
		}
	}

	/*
	 * $(document).on("mouseenter", ".f-dropdown", function(e) { if ($(this).hasClass("mega")) { $("li.has-dropdown").children("a.mega").trigger("click");
	 * $("li.has-dropdown").children("a.mega").addClass('hover'); } else { $(this).parents("li.has-dropdown").children("a").trigger("click");
	 * $(this).parents("li.has-dropdown").children("a").addClass('hover'); }
	 *
	 * }); $(document).on("mouseleave", ".f-dropdown", function(e) { if ($(this).hasClass("mega")) {
	 * $("li.has-dropdown").children("a.mega").removeClass('hover');
	 *  } else { $(this).parents("li.has-dropdown").children("a").removeClass('hover'); } });
	 */
}
 ...
 * $(window).scroll(function(){ var sticky = $('#maintoolbar'),scroll = $(window).scrollTop();
 *
 * if (scroll >= 100) sticky.addClass('squeeze'); else sticky.addClass('squeeze'); });
 */
/** ********* */
...
var delta = 5;
// var navbarHeight = 10;
var navbarHeight = $('#maintoolbar').outerHeight();

$(window).scroll(function(event) {
	if($('#isCustomizedCategory').val() == "false"){
		didScroll = true;
	}
});
...
//Hide Header on on scroll down
function updateHeaderDisplay(evt){
	//initial calculations for stick/unstick of topbar
	if (typeof document.topEdgeOfMenu == 'undefined' || document.topEdgeOfMenu == 0) {
		document.topEdgeOfMenu = $('#topbarcontainer').offset().top;
		document.menuMarginTop = $(".stickytopmenu").css("margin-top");
	}
	...
	var hideHeaderNow = (isScrollingDown && currentScrollTop > 150);
	var menuDistFromTop = document.topEdgeOfMenu - currentScrollTop;
	if(hideHeaderNow){
		$("header").addClass("hideheader");
		$("#topbarcontainer").removeClass("stickbelowheader");
	}
	else {
	...
	//stick/unstuck of topbar
	if (menuDistFromTop <= 0) {
		$("#topbarcontainer").addClass("cat-up");
	}
	if(menuDistFromTop > 150){
		$("#topbarcontainer").removeClass("cat-up");
	}
	else {
		if(hideHeaderNow)
			...
			$("#topbarcontainer").addClass("stickbelowheader");
	}
}
$(window).scroll(updateHeaderDisplay);
function hasScrolled() {
	var st = $(this).scrollTop();

	// Make sure they scroll more than delta
	if (Math.abs(lastScrollTopValue - st) <= delta)
		return;

	// If they scrolled down and are past the navbar, add class .nav-up.
	// This is necessary so you never see what is "behind" the navbar.
	if (st > lastScrollTopValue && st > navbarHeight) {
		// Scroll Down
		$('#maintoolbar').removeClass('nav-down').addClass('nav-up');
	} else {
		// Scroll Up
		if (st + $(window).height() < $(document).height()) {
			$('#maintoolbar').removeClass('nav-up').addClass('nav-down');
			$('#topbarcontainer').removeClass('cat-up').addClass('');
		}
	}
	lastScrollTop = st;
}
