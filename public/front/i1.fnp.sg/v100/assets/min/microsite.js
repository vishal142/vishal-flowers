...
$(document).ready(function() {
	setTimeout(function(){
		  var bannerenable = $('#mainbanner #top').attr('bannersliderEnable');
		  if (bannerenable != 'true') {
		    initBannerConfig();
		  }
		 },0);
});
function initBannerConfig() {
	var currentCatalogId = $("#FNP_CURRENT_CATALOG_ID").length > 0 ? $(
			"#FNP_CURRENT_CATALOG_ID").val() : "sgp";
	var bannerId = "banners";
	if (typeof bannerIdToUse != "undefined") { // Setting bannerIdToUse in meta
												// content Id with name like
												// meta/rakhi etc.
		bannerId = bannerIdToUse;
	} else {
		bannerId = currentCatalogId == "sgp" ? "banners" : "banners/"
				+ currentCatalogId;
	}
	if(currentCatalogId != "sgp"){
		var bannerDataUrl = isPrototype ? "/prototype/assets/js/data/jc.json"
				: AJAX_JSON_URL_LIST_INTL.jcUrl + "?id=" + bannerId;
	}else{
		var bannerDataUrl = isPrototype ? "/prototype/assets/js/data/jc.json"
				: AJAX_JSON_URL_LIST.jcUrl + "?id=" + bannerId;
	}
...
function initBanners() {
	...
	$.each(bannerData, function(index, value) {
		...
		"#FNP_CURRENT_CATALOG_ID").val() : "sgp";
		if(currentCatalogId != "sgp"){
			cdnHost=cdnINTLHostName;
		}
		if (value.fulllink == "Y") {
			image = '<a  class="slide" href="' + value.href + '" title="' + value.alt
					+ '"data-ga-title="' + value.alt + '"> <img  src="'
					+ cdnHost + value.imgsrc + '" alt="' + value.alt
					+ '" usemap="#' + value.id + '" /></a>';
		} else {
			image = '<img class="slide" src="' + cdnHost + value.imgsrc + '" alt="'
					+ value.alt + '" usemap="#' + value.id + '" />';
			maparea = '<map id="' + value.id + '" name="#' + value.id + '">'
			maparea += '<area shape="rect" coords="'
			for (var i = 0; i < value.coord.length; i++) {
				maparea += value.coord[i];
				if (i < value.coord.length - 1)
					maparea += ",";
			}
			maparea += '" alt="' + value.alt + '" href="' + value.href
					+ '" data-ga-element="ga-element" data-ga-title="'
					+ value.alt + '"/>';
			maparea += '</map>'
		}
		if (maparea != null) {
			$('header').append(maparea);
			resetBannerMaps();
			$(window).resize(resetBannerMaps);
		}
		$('#mainbanner').append(image);
	});
		...
		$(window)
			.ready(
					function() {
						var slider = $('#mainbanner');
						slider
								.slick({
									prevArrow : '<div   data-ga-title="previous Arrow" id = "prevArrow" class = "slick-prev" data-ga-element="ga-element" ></div>',
									nextArrow : '<div   data-ga-title="next Arrow" id = "nextArrow" class = "slick-next"  data-ga-element="ga-element"></div>',
									autoplay : false,
									pauseOnHover : true,
									speed : 800,
						...
						setTimeout(function() {
							slider.slick('setOption', 'autoplay', true).slick(
									'slickPlay');
							slider.slick('slickRemove',0);
						}, 1500);
					});
}
function resetBannerMaps() {
	var newWidth = document.body.clientWidth; // we can use jquery also...first lets try this
	$.each(bannerData, function(index, value) {
		var scale = newWidth / value.originalWidth;
		var newLeft = Math.round(value.coord[0] * scale);
		var newTop = Math.round(value.coord[1] * scale);
		var newRight = Math.round(value.coord[2] * scale);
		var newBottom = Math.round(value.coord[3] * scale);
		var newCoords = newLeft + "," + newTop + "," + newRight + ","
				+ newBottom;
		$('#' + value.id + ">area").attr('coords', newCoords);
	});
}
